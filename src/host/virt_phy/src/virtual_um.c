...
/**
 * @brief Join a socket fd to a multicast group.
 */
...
/**
 * @brief Virtual Um file descriptor callback function.
 *
...
static int virt_um_fd_cb(struct osmo_fd *ofd, unsigned int what)
{
	...
	// Check if request is really write request
	if (what & BSC_FD_READ) {
				  ...
				  void (*recv_cb)(struct virt_um_inst *vui, struct msgb *msg))
{
	...
	if (!group)
		group = VIRTUAL_UM_ADDR;

	/* create a socked and bind it to the multicast group. Do NOT
